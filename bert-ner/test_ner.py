...
from BERT_NER import create_model, InputFeatures
...
# init mode and session move something codes outside of function, so that this code will run only once during online
# prediction when predict_online is invoked.
is_training = False
...
label_list = ["O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X", "[CLS]", "[SEP]"]
...
class Pair(object):
    ...
    def __str__(self) -> str:
        line = ['entity:{}'.format(self.__word), 'start:{}'.format(self.__start), 'end:{}'.format(self.__end),
                'merge:{}'.format(self.__merge), 'types:{}'.format(self.__types)]
        return '\t'.join(line)
