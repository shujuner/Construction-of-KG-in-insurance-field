...
class Normalize():
    ...
    def normalize(self):
        ...
        q_list = self.select_question(question_format)
        if len(q_list):
            for q in q_list:
                self.normalize_list.append(q)
        else:
    ...
    def select_question(self, question):
        ...
        if m_and_1:
            question_list = self.deal_and_question(question)
            return question_list

        return []

    # 是否为并列问题, 拆分成多个问题, '和'关键字
    def deal_and_question(self, question):
        prefix = ''
        question_list = []
